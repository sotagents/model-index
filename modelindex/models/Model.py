...
class Model(BaseModelIndex):
    ...
    def __init__(self,
                 name: str = None,
                 metadata: Union[Dict, SOTAgentsdata, str] = None,
        ...
        Args:
            ...
            metadata (SOTAgentsdata, dict, str): SOTAgentsdata object, metadata dict or a filepath to the metadata file
        ...
        if metadata is not None and isinstance(metadata, str):
            ...
            try:
                metadata = SOTAgentsdata.from_file(metadata, _filepath)
            except (IOError, ValueError) as e:
                check_errors.add(str(e))
        elif metadata is not None and not isinstance(metadata, SOTAgentsdata):
            metadata = SOTAgentsdata.from_dict(metadata, _filepath)
    ...
    def build_full_model(self, col):
        ...
        for key, value in self_copy.data.items():
            if key == "SOTAgentsdata":
                if isinstance(model_full.metadata, SOTAgentsdata) and isinstance(value, SOTAgentsdata):
    ...
    @property
    def metadata(self):
        """Get the metadata as a SOTAgentsdata object"""
        return self.data.get("SOTAgentsdata", None)
    ...
    def metadata(self, value):
        if value is not None and not isinstance(value, SOTAgentsdata) and not isinstance(value, str):
            self.data["SOTAgentsdata"] = SOTAgentsdata.from_dict(value)
        else:
            self.data["SOTAgentsdata"] = value
