...
class SOTAgentsdata(BaseModelIndex):
    """SOTAgentsdata for a model."""
    ...
    def _check(self, silent=True):
        if not isinstance(self.data, dict) and not isinstance(self.data, list):
            self.check_errors.add("SOTAgentsdata should be either a list or a dict")

    @staticmethod
    def from_dict(d: Dict, _filepath: str = None):
        """Construct SOTAgentsdata from a dict.
        ...
        dd = d.copy()
        for field_name in SOTAgentsdata.COMMON_FIELDS:
    ...
    def from_file(filepath: str = None, parent_filepath: str = None):
        """Load a SOTAgentsdata from a file.
        ...
        if isinstance(d, dict):
            return SOTAgentsdata.from_dict(d, fullpath)
